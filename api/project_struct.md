...
## api
### cities_osm
1) osm_handler.py - парсинг дорог из osm-файлов
2) osmfetch.py - содержит функции для скачивания osm-файлов
...
1) database.py - содержит определение БД 
2) main.py - главный файл, содержит реализацию запросов
3) models.py - содержит определения классов для взаимодействия с БД 
4) osm_handler.py - дубликат
5) schemas.py -  содержит определения вспомогательных классов 
6) services.py - содержит определения вспомогательных функций
7) street_name_parser.py - содержит определение парсера
### Dockerfile
### requirements.txt
### docker-compose.yaml
...
Функции
    ...
    record_obj_to_wprop,
    record_obj_to_pprop 
используются для преобразования объектов базы данных в списки, которые используются для построения графа дорог.
...
Возвращает четыре списка: список узлов, список ребер, список свойств узлов и список свойств ребер:
        points           - список точек в виде id, longitude и latitude
        edges           - список ребер в виде  id, id_way, from, to и name
        points_prop - список свойств точек в виде  id_point, property и value
        ways_prop   - список свойств дорог каждый из которых содержит id_way, property и value.
